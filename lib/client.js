...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const css = [
      ...
      '.cxsel-addfile{display:none;width:18px;height:18px;flex:none;color:var(--dsw-alias-label-tertiary);cursor:pointer;background:0 0;border:none;border-radius:4px;justify-content:center;align-items:center;padding:0;font-size:14px;line-height:1}',
      '.cxsel-item:hover .cxsel-addfile{display:inline-flex}',
      '.cxsel-addfile:hover{color:var(--dsw-alias-state-business-primary);background:var(--dsw-alias-interactive-bg-hover)}',
    ...
    function parseRef(ref) {
      ...
      const tail = ref.slice(idx + 2)
      if (tail === 'all') return { absPath, wholeFile: true, startLine: 1, endLine: Infinity }
      const m = /^(\d+)-(\d+)$/.exec(tail)
      if (!m) return { absPath, startLine: 1, endLine: 1 }
      return { absPath, startLine: Number(m[1]), endLine: Number(m[2]) }
    }
    function buildBlock(relPath, code, startLine, endLine, ext, wholeFile) {
      const lang = LANG_MAP[ext] || ''
      const body = String(code).replace(/\s+$/, '')
      const header = wholeFile
        ? '// ' + relPath + ' — 整个文件（' + body.split('\n').length + ' 行）'
        : '// ' + relPath + ' — 选中 ' + (endLine - startLine + 1) + ' 行（' + startLine + '-' + endLine + ' 行）'
      return '```' + lang + '\n' + header + '\n' + body + '\n```'
    }
    ...
    function FileView(props) {
      ...
      // Publish a code selection + arm the composer chip. The chip persists
      // until ×, a new pick, or a file switch clears it — never on selection
      // collapse (Codex behaviour).
      useEffect(() => {
      ...
      // Add a WHOLE file to the composer as context (Codex file selection).
      const addFile = useCallback(async (path) => {
        let content = null
        let relPath = relOf(root, path)
        if (current && current.path === path && current.content !== null && current.error === null) {
          content = current.content
          relPath = current.relPath
        } else {
          try {
            const data = await readFile(path)
            if (data && data.content !== null) content = data.content
          } catch { /* fall through */ }
        }
        if (content === null) return
        const ref = refOfFile(path)
        const lineCount = content.split('\n').length
        codeCache.set(ref, { relPath, code: content, lineCount, ext: extOf(path), wholeFile: true })
        pendingInsertRef = ref
        setSelection({ ref, relPath, text: content, lineCount, wholeFile: true, ext: extOf(path) })
      }, [current, root])
              ...
              react_jsx_runtime.jsx('div', {
                ...
                children: [
                  ...
                  react_jsx_runtime.jsx('button', {
                    type: 'button',
                    className: 'cxsel-addfile',
                    title: t ? t('addFile') : 'Add file',
                    'aria-label': t ? t('addFile') : 'Add file',
                    onClick: (e) => { e.stopPropagation(); addFile(entry.path) },
                    children: '+',
                  }),
                ],
              }, 'file-' + entry.path),
            ...
            children: [
              react_jsx_runtime.jsx('div', {
                ...
                children: [
                  ...
                  current && current.content !== null && react_jsx_runtime.jsx('button', { type: 'button', className: 'cxsel-toggle cxsel-addfilebar', onClick: () => addFile(current.path), children: t ? t('addFile') : 'Add file' }),
        ...
        children: react_jsx_runtime.jsx('div', {
          ...
          children: [
            react_jsx_runtime.jsx('span', {
              ...
              title: sel.relPath,
              children: sel.wholeFile
                ? (t ? t('fileSelected', { name: basename(sel.relPath) }) : ('File · ' + basename(sel.relPath)))
                : (t ? t('selectedLines', { count: sel.lineCount, name: basename(sel.relPath) }) : (sel.lineCount + ' lines · ' + basename(sel.relPath))),
            }),
            react_jsx_runtime.jsx('button', {
              type: 'button',
    ...
    function apply(ctx) {
      ...
      ctx.effect(() => ctx.inputTriggers.registerSource({
        ...
        codec: {
          ...
          serialize: async (ref, signal) => {
            ...
            if (cached) {
              return buildBlock(cached.relPath, cached.code, parseRef(ref).startLine, parseRef(ref).endLine, cached.ext, cached.wholeFile)
            }
            const { absPath, startLine, endLine, wholeFile } = parseRef(ref)
            try {
              const data = await readFile(absPath, signal)
              if (data && data.content !== null) {
                if (wholeFile) {
                  return buildBlock(relOf(undefined, absPath), data.content, 1, data.content.split('\n').length, extOf(absPath), true)
                }
            ...
            return wholeFile
              ? '// 文件：' + absPath
              : '// 选中代码：' + absPath + '（' + startLine + '-' + endLine + ' 行）'
          },
        },
      ...
      const syncSelection = (sessionId) => (ref, chip, input) => {
        ...
        const reference = {
          source: 'codex-select',
          ref,
          label: cached.wholeFile ? ('文件 · ' + basename(cached.relPath)) : (cached.lineCount + ' 行 · ' + basename(cached.relPath)),
